<template>
    <div id="app">
            ...
            <div v-if="mode === 'wide'">
                <div style="font-size: 14px;">
                    ...
                    <template v-for="(socket, index) in sockets">
                        <p :key="index" v-if="socket">线程.{{index+1}} {{socket.task ? `正在执行任务.${socket.task.taskIndex}` :
                            '空闲'}}</p>
                    </template>
        ...
        <div id="qmap"></div>
        <radar-progress :show="progressShow" :max-range="max_range" :thread="thread"
                        :percent="progressPercent"></radar-progress>
        <el-dialog
                title="自定义筛选"
            ...
            <div v-for="ylSet in settings.custom_filter" class="filter-list">
                <div class="filter-name">{{ ylSet.setName }}</div>
                <div class="filter-set">
                    <div class="filter-item" v-for="(yl, index) in ylSet.ids" :key="index">
  ...
  export default {
    ...
    mounted() {
      ...
      document.onkeydown = async (e) => {
        ...
        if (keyNum === 65) { // left
          await axios.get(`http://localhost:8080/gps/lon/${ this.location.longitude - distance }/lat/${ this.location.latitude }`)
          this.location.longitude -= distance
        } else if (keyNum === 68) { // right
          await axios.get(`http://localhost:8080/gps/lon/${ this.location.longitude + distance }/lat/${ this.location.latitude }`)
          this.location.longitude += distance
        } else if (keyNum === 83) { // down
          await axios.get(`http://localhost:8080/gps/lon/${ this.location.longitude }/lat/${ this.location.latitude - distance }`)
          this.location.latitude -= distance
        } else if (keyNum === 87) { // up
          await axios.get(`http://localhost:8080/gps/lon/${ this.location.longitude }/lat/${ this.location.latitude + distance }`)
          this.location.latitude += distance
        }
